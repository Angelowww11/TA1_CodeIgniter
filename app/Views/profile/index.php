<?= view('partials/app_header', ['title' => $title]) ?>

<section class="page-intro" aria-labelledby="profile-title">
    <p class="kicker">Demo account</p>
    <div class="page-intro-grid">
        <h1 id="profile-title">Profile</h1>
        <p>The single user record stored in the users table and retrieved through UserModel.</p>
    </div>
</section>

<section class="content-section">
    <?php if ($user === null): ?>
        <div class="empty-state"><h2>No user found</h2><p>Run the database seeder to create the required demo account.</p></div>
    <?php else: ?>
        <article class="profile-card">
            <div class="profile-monogram" aria-hidden="true"><?= esc(strtoupper(substr($user['full_name'], 0, 1))) ?></div>
            <div class="profile-main">
                <p class="profile-label">System developer</p>
                <h2><?= esc($user['full_name']) ?></h2>
                <a href="mailto:<?= esc($user['email'], 'attr') ?>"><?= esc($user['email']) ?></a>
            </div>
            <dl class="profile-details">
                <div><dt>Username</dt><dd><?= esc($user['username']) ?></dd></div>
                <div><dt>Member since</dt><dd><?= esc(date('F j, Y', strtotime($user['created_at']))) ?></dd></div>
            </dl>
        </article>
    <?php endif ?>
</section>

<?= view('partials/app_footer') ?>
